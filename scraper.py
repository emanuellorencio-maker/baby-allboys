...
def sacar_direcciones(lineas: List[str], fixture: List[Dict], nombre_equipo: str) -> Dict[str, str]:
    inicio = indice_linea(lineas, "Nombre del Equipo Dirección Localidad T.")
    if inicio == -1:
        inicio = indice_linea(lineas, "Nombre del Equipo Direccion Localidad T.")
    fin = next((i for i, l in enumerate(lineas[inicio + 1:], start=inicio + 1) if limpiar_linea(l).upper().startswith("F.T.EQUIPOS")), -1)
    if inicio == -1:
        return {}

    equipos = extraer_todos_los_equipos(lineas)
    equipos.extend([item["local"] for item in fixture] + [item["visitante"] for item in fixture] + [nombre_equipo])
    equipos_ordenados = sorted({normalizar_equipo(x) for x in equipos if x}, key=len, reverse=True)

    bloque = lineas[inicio + 1: fin if fin != -1 else None]
...
def procesar_zona(clave: str, info: Dict):
    ...
    guardar_json(clave, "direcciones", direcciones)
