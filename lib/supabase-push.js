...
function debugEnv() {
  ...
  return {
    ...
    supabaseUrlLooksValid: /^https:\/\/[a-z0-9-]+\.supabase\.co$/i.test(supabaseUrl.replace(/\/+$/, "")),
    serviceRoleKeyLooksJwt: /^eyJ/.test(serviceRoleKey) && serviceRoleKey.split(".").length === 3,
    hasVapidPublicKey: Boolean(envValue("VAPID_PUBLIC_KEY")),
    hasVapidPrivateKey: Boolean(envValue("VAPID_PRIVATE_KEY")),
    hasVapidSubject: Boolean(envValue("VAPID_SUBJECT")),
    hasAdminToken: Boolean(envValue("ADMIN_TOKEN")),
