...
const RESULTADO_START_COLUMN = "I";
const RESULTADO_END_COLUMN = "J";
...
function normalizeResultValue(value) {
  ...
  if (["GP", "NP", "PP"].includes(normalized)) return normalized;
  return normalized;
}

function formatResultValues(result) {
  if (!result) return ["", ""];
  return [normalizeResultValue(result.local), normalizeResultValue(result.visitante)];
}
...
async function actualizarResultadosEnSheet({ zona, fechaId, partido }) {
  ...
  try {
    ...
    entries.forEach(([division, result]) => {
      ...
      updates.push({
        range: `${quoteSheetName(selected.candidate.sheetTitle)}!${RESULTADO_START_COLUMN}${selected.candidate.rowNumber}:${RESULTADO_END_COLUMN}${selected.candidate.rowNumber}`,
        values: [formatResultValues(result)],
      });
    });
